...
const PublicChat = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const sessionRes = await axios.get(`/api/sessions/public/${sessionId}`);
        setSession(sessionRes.data.data);
        
        // Fetch messages
        const messagesRes = await axios.get(`/api/sessions/public/${sessionId}/messages`);
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      // Send message to API
      const res = await axios.post(`/api/sessions/public/${sessionId}/messages`, {
        message: newMessage,
  ...
  const theme = {
    palette: {
      ...
      background: {
        ...
        chat: mode === 'light' ? '#f5f5f5' : '#121212',
        message: {
          user: mode === 'light' ? '#e3f2fd' : '#1a3f5e',
          bot: mode === 'light' ? '#ffffff' : '#1e1e1e'
        }
      },
